...
enum {
    ...
    MUXER_INSUFFICIENT_MEMORY =
            -6,                   /* memory not enough, causing general memory allocation failure */
...
read
    ...
    Return value:
        ...
        If this number differs from the size parameter, either an error occured or the EOF was
reached.
...
check_available_bytes
    ...
    The muxer can decide not to read if cached data is not enough and so avoid reading failure in
unexpected context. For a local file, any bytes request from the muxer can be met as long as it's
within the file range.

    Arguments:
        handle [in] Handle of the file.
        bytes_requested [in]    Bytes requested for further parsing. This information can help the
application to cache enough data before calling muxer API next time. If the muxer can not know the
...
typedef struct _FslFileStream {
    ...
    uint32 (*Write)(FslFileHandle handle, void* buffer, uint32 size,
 ...
 * And in its DLL entry point "FslMuxerInit", it shall set the not-implemented function pointers to
 *NULL.
 *
 *********************************************************************************************************/
 ...
 ***************************************************************************************/
/* all mandatory */
typedef const char* (*FslMuxerApiVersionInfo)(uint32* mainVersion, uint32* minorVersion);

typedef const char* (*FslMuxerVersionInfo)();

typedef int32 (*FslCreateMuxer)(bool isLive, FslFileStream* streamOps, MuxerMemoryOps* memOps,
                                void* context, FslMuxerHandle* muxerHandle);
 ...
 ************************************************************************************************************/
/* mandatory */
typedef int32 (*FslMuxerSetMovieProperty)(FslMuxerHandle muxerHandle, PropertyID propertyId,
                                          DataFormat propertyFormat, void* propertyData,
                                          uint32 propertyDataLength);

/* optional */
typedef int32 (*FslMuxerSetMovieMetadata)(FslMuxerHandle muxerHandle, MetadataID metadataId,
                                          DataFormat metadataFormat, void* metadata,
...
/* mandatory */
typedef int32 (*FslMuxerAddTrack)(FslMuxerHandle muxerHandle, uint32 mediaType, uint32 codecType,
                                  uint32 codecSubType, uint32* trackId);
 ...
 ************************************************************************************************************/
/* mandatory */
typedef int32 (*FslMuxerSetTrackProperty)(FslMuxerHandle muxerHandle, uint32 trackId,
                                          PropertyID propertyId, DataFormat propertyFormat,
                                          void* propertyData, uint32 propertyDataLength);

/* optional */
typedef int32 (*FslMuxerSetTrackMetadata)(FslMuxerHandle muxerHandle, uint32 trackId,
                                          MetadataID metadataId, DataFormat metadataFormat,
                                          void* metadata, uint32 metadataLength);

/* mandatory */
typedef int32 (*FslMuxerAddSample)(FslMuxerHandle muxerHandle, uint32 trackId, uint8* sampleBuffer,
                                   uint32 sampleSize, uint64 pts, uint64 dts, uint64 duration,
                                   uint32 flags);
...
If the queried API is not implemented, the muxer shall set funtion pointer to NULL and return
MUXER_SUCCESS. */

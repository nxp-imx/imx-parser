 ...
 * Please use data types that can explicitly tell the data length and asign them the listed enum
 * values. It's because different compilers can treat enum as different types such as integer or
 * unsinged char. If the parser library and plug-in(filter) are built by different compliers, the
 * data length mismatch will cause error.
 */
...
#define PARSER_UNKNOWN_DURATION                                                                 \
    0 /* Unknown movie, track or sample duration.                                               \
        ...
        The plug-in and the core parser shall just try to read as many sample as possible until \
        EOF.*/

#define PARSER_UNKNOWN_TIME_STAMP                                                \
    (-1) /* The time stamp is unknown. Usually used when the exact audio samples \
...
enum {
    ...
    PARSER_INSUFFICIENT_MEMORY =
            -7, /* memory not enough, causing general memory allocation failure */
    ...
    /* error concealment */
    PARSER_ERR_CONCEAL_FAIL =
            -22, /* Error in bitstream and no sample can be found by error concealment.
    ...
    PARSER_ERR_TRACK_DISABLED = -30, /* The track is disabled and no media samples can be read from
                                        it. Only enabled track can output samples.*/
    PARSER_ERR_INVALID_READ_MODE = -32, /* The reading mode is invalid, or some operation is illegal
                                           under current reading mode */
};
...
#define FLAG_SAMPLE_ERR_CONCEALED \
    0X02 /* This sample is got by error concealment, such as searching the bitstream.*/

#define FLAG_SAMPLE_SUGGEST_SEEK                                                 \
    0X04 /* A seeking is suggested. Although sample is got by error concealment, \
         A/V sync may be impacted.                                               \
         If the file is seekable, a seeking on all tracks can save the A/V sync.*/
#define FLAG_SAMPLE_NOT_FINISHED \
    0X08 /* Sample is NOT finished at this call, large samples can be output in several calls. */

#define FLAG_UNCOMPRESSED_SAMPLE                                                              \
    0X10 /* This is a uncompressed sample.                                                    \
           Warning:                                                                           \
...
#define FLAG_SAMPLE_PCR_INFO \
    0x80 /* This is a sample of PCR data, the data is stored in struct PCRInfo type */

#define FLAG_SAMPLE_H264_SEI_POS_DATA                                                           \
    0x100 /* This is a sample of h264 sei position data, the position data gives the offset and \
           * size of the sei nalu in the h264 frame it belongs to */

#define FLAG_SAMPLE_COMPRESSED_SAMPLE 0x200  // drm sample flag, need to call drm api to get drm
                                             // info

#define FLAG_SAMPLE_AUDIO_PRESENTATION_CHANGED \
    0x400  // audio presentation is changed , need to call getMetaData to update

/*********************************************************************
 * seeking flags :
 when to seek, must set one of the following flags
 ********************************************************************/
#define SEEK_FLAG_NEAREST                                                                       \
    0X01 /* Default flag. The actual seeked time shall be nearest one to the given time (can be \
            later or earlier)*/
#define SEEK_FLAG_NO_LATER 0X02 /* The actual seeked time shall be no later than the given time */
#define SEEK_FLAG_NO_EARLIER \
    0X03 /* The actual seeked time shall be no earlier than the given time */
#define SEEK_FLAG_FUZZ                                                        \
    0X04 /* Reserved. No accurate request on time but request quick response. \
            Parsers shall optimize performances with other flags.*/
#define SEEK_FLAG_CLOSEST                                                                        \
    0X05 /* this flag is only used for android when read options is ReadOptions::SEEK_CLOSEST in \
            mp4 parser. */
...
// file source should be read in sequence. You should not read from random position even if the file
// is seekable
 ...
 ********************************************************************/
// if set the flag, stream format was avc if codec data is presented, and format is bytestream if
// codec data is not found.
...
read
    ...
    Return value:
        ...
        If this number differs from the size parameter, either an error occurred or the EOF was
reached.
...
seek
    ...
    Arguments:
        ...
        offset [in] The offset.
                    To move to a position before the end-of-file, you need to pass a negative value
in offset and set whence to SEEK_END.
...
tell
    ...
    Return value:
        ...
        If error occurs or this feature can not be supported (eg. broadcast application), returns
-1.
...
check_available_bytes
    ...
    The parser can decide not to read if cached data is not enough and so avoid reading failure in
unexpected context. For a local file, any bytes request from the parser can be met as long as it's
within the file range.

    Arguments:
        handle [in] Handle of the file.
        bytes_requested [in]    Bytes requested for further parsing. This information can help the
application to cache enough data before calling parser API next time. If the parser can not know the
exact data size needed, set it to 0.

    Return value:
        If the file source can always meet the data reading request unless EOF (eg. a local file or
a pull-mode live source), returns the data size from the current file pointer to the file end.
...
Usually, the core parser requests an output buffer, fill the media data and return it to the
application on GetNextSample(). But, on flushing (eg. on seek or deletion), the core parser need
explicitly release all buffers not returned yet.
...
RequestBuffer
    ...
    Arguments:
        ...
        size [in,out] The requested buffer size as input, and the size actually got as output, both
in bytes. The actually got size can be larger than the requested size, and the parser can make full
use of the buffer.
...
typedef int32 (*FslCreateParser)(bool isLive, FslFileStream* streamOps, ParserMemoryOps* memOps,
...
/* optional */
typedef int32 (*FslCreateParser2)(uint32 flags, FslFileStream* streamOps, ParserMemoryOps* memOps,
                                  ParserOutputBufferOps* outputBufferOps, void* context,
                                  FslParserHandle* parserHandle);
 ...
 ***************************************************************************************/
/* optional */
typedef int32 (*FslParserInitializeIndex)(
...
typedef int32 (*FslParserExportIndex)(FslParserHandle parserHandle, uint8* buffer, uint32* size);
...
typedef int32 (*FslParserGetUserData)(FslParserHandle parserHandle, uint32 userDataId,
                                      uint16** unicodeString, uint32* stringLength);

typedef int32 (*FslParserGetMetaData)(FslParserHandle parserHandle, UserDataID userDataId,
                                      UserDataFormat* userDataFormat, uint8** userData,
                                      uint32* userDataLength);

typedef int32 (*FslParserGetNumTracks)(FslParserHandle parserHandle,
                                       uint32* numTracks); /* single program interface */

typedef int32 (*FslParserGetNumPrograms)(FslParserHandle parserHandle,
                                         uint32* numPrograms); /* multiple program interface */
typedef int32 (*FslParserGetProgramTracks)(FslParserHandle parserHandle, uint32 programNum,
                                           uint32* numTracks, uint32** ppTrackNumList);

typedef int32 (*FslParserGetPCR)(FslParserHandle parserHandle, uint32 programNum, uint64* usPCR);
 ...
 *
 ************************************************************************************************************/
typedef int32 (*FslParserGetTrackType)(FslParserHandle parserHandle, uint32 trackNum,
                                       uint32* mediaType, uint32* decoderType,
                                       uint32* decoderSubtype);

typedef int32 (*FslParserGetTrackDuration)(
        FslParserHandle parserHandle, uint32 trackNum,
        uint64* usDuration); /* Duration 0 means an empty track */

typedef int32 (*FslParserGetLanguage)(FslParserHandle parserHandle, uint32 trackNum,
                                      uint8* threeCharCode);

/* optional */
typedef int32 (*FslParserGetBitRate)(FslParserHandle parserHandle, uint32 trackNum,
                                     uint32* bitrate);

typedef int32 (*FslParserGetDecSpecificInfo)(FslParserHandle parserHandle, uint32 trackNum,
                                             uint8** data, uint32* size);

typedef int32 (*FslParserGetTrackExtTag)(FslParserHandle parserHandle, uint32 trackNum,
 ...
 ************************************************************************************************************/
/* mandatory */
typedef int32 (*FslParserGetVideoFrameWidth)(FslParserHandle parserHandle, uint32 trackNum,
                                             uint32* width);

typedef int32 (*FslParserGetVideoFrameHeight)(FslParserHandle parserHandle, uint32 trackNum,
                                              uint32* height);

/* optional */
typedef int32 (*FslParserGetVideoFrameRate)(FslParserHandle parserHandle, uint32 trackNum,
                                            uint32* rate, uint32* scale);

typedef int32 (*FslParserGetVideoFrameRotation)(FslParserHandle parserHandle, uint32 trackNum,
                                                uint32* rotation);
...
typedef int32 (*FslParserGetVideoHDRColorInfo)(FslParserHandle parserHandle, uint32 trackNum,
                                               VideoHDRColorInfo* pInfo);

typedef int32 (*FslParserGetVideoDisplayWidth)(FslParserHandle parserHandle, uint32 trackNum,
                                               uint32* width);

typedef int32 (*FslParserGetVideoDisplayHeight)(FslParserHandle parserHandle, uint32 trackNum,
                                                uint32* height);

typedef int32 (*FslParserGetVideoFrameCount)(FslParserHandle parserHandle, uint32 trackNum,
                                             uint32* count);

typedef int32 (*FslParserGetVideoThumbnailTime)(FslParserHandle parserHandle, uint32 trackNum,
                                                uint64* outTs);

typedef int32 (*FslParserGetVideoScanType)(FslParserHandle parserHandle, uint32 trackNum,
                                           uint32* scanType);
 ...
 ************************************************************************************************************/
/* mandatory */
typedef int32 (*FslParserGetAudioNumChannels)(FslParserHandle parserHandle, uint32 trackNum,
                                              uint32* numchannels);

typedef int32 (*FslParserGetAudioSampleRate)(FslParserHandle parserHandle, uint32 trackNum,
                                             uint32* sampleRate);

typedef int32 (*FslParserGetAudioBitsPerSample)(FslParserHandle parserHandle, uint32 trackNum,
                                                uint32* bitsPerSample); /* bit depth */

/* optional */
typedef int32 (*FslParserGetAudioBlockAlign)(FslParserHandle parserHandle, uint32 trackNum,
                                             uint32* blockAlign);

typedef int32 (*FslParserGetAudioBitsPerFrame)(FslParserHandle parserHandle, uint32 trackNum,
                                               uint32* bits_per_frame); /* for Real audio */

typedef int32 (*FslParserGetAudioChannelMask)(FslParserHandle parserHandle, uint32 trackNum,
                                              uint32* channelMask); /* for WMA audio */
typedef int32 (*FslParserGetAudioMpeghInfo)(FslParserHandle parserHandle, uint32 trackNum,
                                            uint32* profileLevelIndication,
                                            uint32* referenceChannelLayout,
                                            uint32* compatibleSetsSize, uint8** compatibleSets);
typedef int32 (*FslParserGetSampleInfo)(FslParserHandle parserHandle, uint32 trackNum,
 ...
 ************************************************************************************************************/
/* mandatory */
typedef int32 (*FslParserGetTextTrackWidth)(FslParserHandle parserHandle, uint32 trackNum,
                                            uint32* width);

typedef int32 (*FslParserGetTextTrackHeight)(FslParserHandle parserHandle, uint32 trackNum,
                                             uint32* height);

typedef int32 (*FslParserGetTextTrackMime)(FslParserHandle parserHandle, uint32 trackNum,
 ...
 *
 ************************************************************************************************************/
typedef int32 (*FslParserGetImageInfo)(FslParserHandle parserHandle, uint32 trackNum,
                                       ImageInfo** pInfo);
...
typedef int32 (*FslParserEnableTrack)(FslParserHandle parserHandle, uint32 trackNum, bool enable);

typedef int32 (*FslParserGetNextSample)(
        FslParserHandle parserHandle, uint32 trackNum, uint8** sampleBuffer, void** bufferContext,
        uint32* dataSize, uint64* usStartTime, uint64* usDuration,
        uint32* sampleFlags); /* Only for track-based sample reading. The application tell which
                                 track to read.*/

/* optional */
typedef int32 (*FslParserGetNextSyncSample)(FslParserHandle parserHandle, uint32 direction,
                                            uint32 trackNum, uint8** sampleBuffer,
                                            void** bufferContext, uint32* dataSize,
                                            uint64* usStartTime, uint64* usDuration,
                                            uint32* flags); /* only for trick mode on video track */

typedef int32 (*FslParserGetFileNextSample)(
        FslParserHandle parserHandle, uint32* trackNum, uint8** sampleBuffer, void** bufferContext,
        uint32* dataSize, uint64* usStartTime, uint64* usDuration,
        uint32* sampleFlags); /* Only for file-based sample reading. The parser tell which track is
                                 being read.*/

/* optional */
typedef int32 (*FslParserGetFileNextSyncSample)(
        FslParserHandle parserHandle, uint32 direction, uint32* trackNum, uint8** sampleBuffer,
        void** bufferContext, uint32* dataSize, uint64* usStartTime, uint64* usDuration,
        uint32* flags); /* only for trick mode on video track */

// crypto info for android widevine
typedef int32 (*FslParserGetSampleCryptoInfo)(FslParserHandle parserHandle, uint32 trackNum,
                                              uint8** iv, uint32* ivSize, uint8** clearBuffer,
                                              uint32* clearSize, uint8** encryptedBuffer,
                                              uint32* encryptedSize);

/* mandatory */
typedef int32 (*FslParserSeek)(FslParserHandle parserHandle, uint32 trackNum, uint64* usTime,
                               uint32 flag);

/* optional */
typedef int32 (*FslParserGetAudioPresentationNum)(FslParserHandle parserHandle, uint32 trackNum,
                                                  int32* presentationNum);

/* optional */
typedef int32 (*FslParserGetAudioPresentationInfo)(FslParserHandle parserHandle, uint32 trackNum,
